...
void bf_init(bitfield_t *bitfield, unsigned int size)
{
	bitfield->count = size/(8*sizeof(unsigned char)) + sizeof(unsigned char);
	bitfield->bits = (unsigned char*)malloc(count);
	assert(bitfield->bits != NULL);
	bzero(bitfield->bits, bitfield->count*sizeof(unsigned char));
}

void bf_destroy(bitfield_t *bitfield)
{
	free(bitfield->bits);
	bitfield->bits = NULL;
	bitfield->count = 0;
...
void bf_set(bitfield_t *bitfield, unsigned int bit)
{
	unsigned int word_offset = bit/sizeof(unsigned char);
	assert(word_offset < bitfield->count);
	unsigned int bit_offset = bit % sizeof(unsigned char);
	unsigned char mask = 1 << bit_offset;

	bitfield->bits[word_offset] |= mask;
}

bool bf_test(bitfield_t *bitfield, unsigned int bit)
{
	unsigned int word_offset = bit/sizeof(unsigned char);
	assert(word_offset < bitfield->count);
	unsigned int bit_offset = bit % sizeof(unsigned char);
	...
	return bitfield->bits[word_offset] & mask;
}

void bf_and(bitfield_t *dst, bitfield_t *src)
{
	int i;
...
void bf_or(bitfield_t *dst, bitfield_t *src)
{
	int i;
 ...
 */
PyObject* common_sub(tree_handle_t *tree, int min_len, int min_occ)
{
	params_t p;
...
common_substrings(PyObject* self, PyObject* args)
{
	...
	return common_sub(handle, min_len, min_occ);
}
